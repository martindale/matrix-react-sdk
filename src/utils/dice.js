...
class DiceRoller {
   ...
   */
  parse (formula) {
    let matches = formula.match(/^(\d+)?d(\d+)([+-]\d+)?$/i);
    if (matches === null || matches[2] == 0) {
      return null;
    }

    let rolls    = (matches[1] !== undefined) ? (matches[1] - 0) : 1;
    let sides    = (matches[2] !== undefined) ? (matches[2] - 0) : 0;
    let modifier = (matches[3] !== undefined) ? (matches[3] - 0) : 0;
   ...
   */
  validate (formula) {
    return (this.parse(formula) === null) ? false : true ;
  }
}
